...
void MultiModalPRM::InitializeExplicit(ExplicitMMCSpace* cspace)
{
  ...
  //copy the graph structure into planningGraph
  planningGraph.Resize(cspace->modeGraph.nodes.size());
  for(size_t i=0;i<cspace->modeGraph.nodes.size();i++) {
    Graph::EdgeIterator<CSpace*> e;
    for(cspace->modeGraph.Begin(i,e);!e.end();e++) {
      planningGraph.AddEdge(i,e.target());
    }
  }
  //initialize planningGraph
  for(size_t i=0;i<cspace->modeGraph.nodes.size();i++) {
...
MultiModalPRM::TransitionIndex MultiModalPRM::NodeToTransitionIndex(int mode,int roadmapIndex) const
{
  ...
  fprintf(stderr,"Transition not found\n");
  abort();
  return index;
}
